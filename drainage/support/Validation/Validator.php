...
/**
 * Class Validator
 * 此类覆盖了\Illuminate\Validation\Validator类
 * 由于原Laravel提供的Validator验证机制与错误不适用
 * 因此用这个类覆盖原类
 * 1. 修改了validate方法: 遇错则停, 因此每次验证只会存在一个error
 * 2. 修改了addError方法: 直接返回出错的attribute名
 * 3. 新增errorAttribute属性: 直接记录出错的属性名
 *
 * Factory的resolver绑定见\App\Providers\AppServiceProvider::registerValidator()
 *
...
class Validator extends BaseValidator
{
     ...
     * @return array
     */
    public function parseData(array $data, $arrayKey = null)
    {
        if (is_null($arrayKey)) {
            $this->files = [];
        }
...
//                }
        ...
        return empty($this->errorAttribute);
    }
